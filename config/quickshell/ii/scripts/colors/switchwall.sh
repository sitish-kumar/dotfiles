...
switch() {
    ...
    # Skip terminal color application when called from settings (--noswitch)
    # to avoid SIGUSR1 to kitty / PTY writes causing unwanted workspace focus shift.
...
main() {
    ...
    if [[ -z "$imgpath" && -z "$color_flag" && -z "$noswitch_flag" ]]; then
        cd "$(xdg-user-dir PICTURES)/Wallpapers/wallpaper" 2>/dev/null || cd "$(xdg-user-dir PICTURES)/Wallpapers" 2>/dev/null || cd "$(xdg-user-dir PICTURES)" || return 1
