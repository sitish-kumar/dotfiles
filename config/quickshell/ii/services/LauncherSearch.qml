    ...
    property list<var> results: {
        ...
        if (root.query.startsWith(Config.options.search.prefix.clipboard)) {
            ...
            const makeEmoji = (emoji, name) => resultComp.createObject(null, {
                ...
                verb: Translation.tr("Type"),
                type: Translation.tr("Emoji"),
                execute: () => {
                    EmojiHistory.record(emoji);
                    Quickshell.clipboardText = emoji;  // always copy to clipboard
                    // ...and also type it into the app once the launcher closes
                    // and focus returns (so you get both input AND copy).
                    Quickshell.execDetached(["bash", "-c", 'sleep 0.15; wtype "$1"', "emoji", emoji]);
                }
            });
